...
<div class="left-content container">
	...
	<form id="form-sanpham" action="">
		...
		<div class="col-md-5 col-sm-12 form-group">
			...
			<input type="text" name="giatien" id="giatien" class="form-control" placeholder="nhập vào giá"/><br>
			
			<label for="danhMucSanPham">Danh mục sản phẩm :3</label>
			...
			<label class="radio-inline"><input id="rd-nu" type="radio" value="nữ" name="gianhcho">nữ</label><br>
			
			<br><label for="mota">Mô tả</label>
			<textarea rows="10" name="mota" id="mota" class="form-control" placeholder="Mô tả SP"></textarea><br>
			
			<label for="hinhanh">Hình ảnh</label>
			<input type="file" name="hinhanh" id="hinhanh" class="form-control form-control-file"/><br>
	
			<div id="containerchitietsanpham">
				<div class="chitietsanpham">
					<span>Chi Tiết</span><br>
				
					<br><select name="mausanpham" class="form-control" id="mausanpham">
						 <c:forEach var="mausanpham" items="${listmau }">
						 	<option value="${mausanpham.getMamau() }">${mausanpham.getTenmau() }</option>
						 </c:forEach>
					  </select><br>
					  
					  <select name="sizesanpham" class="form-control" id="sizesanpham">
						 <c:forEach var="sizesanpham" items="${listsize }">
						 	<option value="${sizesanpham.getMasize() }">${sizesanpham.getSize() }</option>
						 </c:forEach>
					  </select><br>
					  
					  <label for="soluong">Số lượng</label>
					  <input min="1" value="1" type="number" name="soluong" id="soluong" class="form-control" placeholder="nhập số lượng sản phẩm"/><br>
		...
		<div class="col-md-7 col-sm-12">
			...
			<table id="table-sanpham" class="table table-striped">
				<thead>
				    <tr>
				      <th scope="col">
				      		<input id="check-all" class="form-check-input checkbox-sanpham" type="checkbox">
				  ...
				  <tbody>
				  	<c:forEach var="value" items="${listsanpham }">
							<tr>
								<td>
									<input class="form-check-input checkbox-sanpham" type="checkbox" value="${value.getMasanpham() }" id="defaultCheck1">
									 <label class="form-check-label" for="defaultCheck1">
										    Chọn
									 </label>
								</td>
								<td class="sanpham" data-masp="${value.getMasanpham() }">${value.getTensanpham() }</td>
								<td class="giatien-giohang">${value.getGiatien() }</td>
								<td class="gianhcho">${value.getGianhcho() }</td>
								<td style="color: white;" class="btn btn-primary capnhatsanpham" data-id="${value.getMasanpham() }">Sửa</td>
							</tr>
					</c:forEach>
				  </tbody>
			</table>
                           ...
                           <div class="menu">
									<ul id="menu" >
										<li><a href="index.html"><i class="fa fa-tachometer"></i> <span>Dashboard</span><div class="clearfix"></div></a></li>
										 ...
										 <li id="menu-academico" ><a href='<c:url value="/themsanpham"/>'><i class="fa fa-credit-card nav_icon"></i><span>Sản Phẩm</span><div class="clearfix"></div></a></li>
